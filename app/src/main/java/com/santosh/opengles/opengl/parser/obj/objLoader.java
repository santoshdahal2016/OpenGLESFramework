...
public class objLoader {
	...
	private static String getParameter(Uri uri, String parameter) {
		...
		try  {
			...
			br = new BufferedReader(new InputStreamReader(IOHelper.getInputStream(uri)));
			String line;
		...
		public boolean addFace(String line) {
			try {
				...
				for (int i = 0, faceIndex = 0; i < numTokens; i++, faceIndex++) {
					...
					if (objLoader.triangleMode == GLES20.GL_TRIANGLE_FAN) {
						if (faceIndex == 0){
					...
					else if (objLoader.INDEXES_START_AT_1) {
						vertIdx--;
